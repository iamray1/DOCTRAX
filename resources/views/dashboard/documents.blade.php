...
function renderDrawer(doc) {
    ...
    if (!logs.length) {
        ...
        Array.from(logs).reverse().forEach(function(log, idx) {
            ...
            var latestClass = latestStatus === 'completed' ? 'c-done' : (/return|resubmit/.test(latestStatus) ? 'c-danger' : 'c-latest');
            var dc = isVoided ? 'c-voided' : (isLatest ? latestClass : dotClass(log.status_after));
